...
int bst_path_sum(int sum, struct bst* bst) {
  ...
  // subtracting current node value from current sum
  sum -= bst->root->val;
...
/*
 * Helper function that traverses the BST in reverse in-order and pushes each 
 * node value onto the stack, so that the nodes can later be popped from the 
 * stack in-order. Works by recursively traversing the tree by:
 *    right subtree -> root -> left subtree
 *
 * Params:
 * 	 root - the root node of the BST to get node values from
 *
 * 	 stack - the stack to push values onto
 *
 * Return:
 *   void
 */
void reverse_in_order(struct bst_node* root,struct stack* stack) {
  ...
  // recursive case: not yet at the end of a branch
